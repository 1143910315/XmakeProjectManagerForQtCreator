...
const char COMMON_ERROR_PATTERN[] = "^XMake Error at (.*?):([0-9]*?)( \\((.*?)\\))?:";
const char NEXT_SUBERROR_PATTERN[] = "^XMake Error in (.*?):";
const char COMMON_WARNING_PATTERN[] = "^XMake Warning (\\(dev\\) )?at (.*?):([0-9]*?)( \\((.*?)\\))?:";
const char LOCATION_LINE_PATTERN[] = ":(\\d+?):(?:(\\d+?))?$";
const char SOURCE_LINE_AND_FUNCTION_PATTERN[] = "  (.*?):([0-9]*?)( \\((.*?)\\))";

XMakeParser::XMakeParser()
{
...
void XMakeParser::setSourceDirectory(const FilePath &sourceDir)
{
...
FilePath XMakeParser::resolvePath(const QString &path) const
{
    if (m_sourceDirectory)
...
OutputLineParser::Result XMakeParser::handleLine(const QString &line, OutputFormat type)
{
    ...
    case NONE: {
        ...
        if (match.hasMatch()) {
            ...
            return {Status::InProgress};
        } else if (trimmedLine.endsWith(QLatin1String("in xmake code at"))) {
            ...
            return Status::InProgress;
        } else if (trimmedLine.startsWith("XMake Error: ")) {
            m_lastTask = BuildSystemTask(Task::Error, trimmedLine.mid(13));
            m_lines = 1;
            return Status::InProgress;
        } else if (trimmedLine.startsWith("-- ") || trimmedLine.startsWith(" * ")) {
            // Do not pass on lines starting with "-- " or "* ". Those are typical XMake output
            return Status::InProgress;
...
class XMakeParserTest final : public QObject
{
    Q_OBJECT

private slots:
    void testXMakeParser_data();
    void testXMakeParser();
};

void XMakeParserTest::testXMakeParser_data()
{
    ...
    QTest::newRow("add custom target")
            << QString::fromLatin1("XMake Error at src/1/app/XMakeLists.txt:70 (add_custom_target):\n"
                                   "  Cannot find source file:\n\n"
                                   "    unknownFile.qml\n\n"
                                   "  Tried extensions .c .C .c++ .cc .cpp .cxx .m .M .mm .h .hh .h++ .hm .hpp\n"
                                   "  .hxx .in .txx\n\n\n"
                                   "XMake Error in src/1/app/XMakeLists.txt:\n"
                                   "  Cannot find source file:\n\n"
                                   "    XMakeLists.txt2\n\n"
                                   "  Tried extensions .c .C .c++ .cc .cpp .cxx .m .M .mm .h .hh .h++ .hm .hpp\n"
                                   "  .hxx .in .txx\n\n")
            ...
            << (Tasks()
                << BuildSystemTask(Task::Error,
                                   ...
                                   "Tried extensions .c .C .c++ .cc .cpp .cxx .m .M .mm .h .hh .h++ .hm .hpp\n"
                                   ".hxx .in .txx",
                                    FilePath::fromUserInput("src/1/app/XMakeLists.txt"), 70)
                << BuildSystemTask(Task::Error,
                                   "Cannot find source file:\n\n"
                                   "  XMakeLists.txt2\n\n"
                                   "Tried extensions "
                                   ...
                                   ".hxx .in .txx",
                                   FilePath::fromUserInput("src/1/app/XMakeLists.txt"), -1))
            << QString();

    QTest::newRow("add subdirectory")
            << QString::fromLatin1("XMake Error at src/1/XMakeLists.txt:8 (add_subdirectory):\n"
            ...
            << (Tasks()
                << BuildSystemTask(Task::Error,
                                   "add_subdirectory given source \"app1\" which is not an existing directory.",
                                   FilePath::fromUserInput("src/1/XMakeLists.txt"), 8))
            << QString();

    QTest::newRow("unknown command")
            << QString::fromLatin1("XMake Error at src/1/XMakeLists.txt:8 (i_am_wrong_command):\n"
                                   "  Unknown XMake command \"i_am_wrong_command\".\n\n")
            << OutputParserTester::STDERR
            << QString() << QString()
            << (Tasks()
                << BuildSystemTask(Task::Error,
    ...
    QTest::newRow("incorrect arguments")
            << QString::fromLatin1("XMake Error at src/1/XMakeLists.txt:8 (message):\n"
    ...
    QTest::newRow("xmake error")
            << QString::fromLatin1("XMake Error: Error in xmake code at\n"
                                   "/test/path/XMakeLists.txt:9:\n"
            ...
            << (Tasks()
                << BuildSystemTask(Task::Error,
                                   "Parse error.  Expected \"(\", got newline with text \"\n\".",
                                   FilePath::fromUserInput("/test/path/XMakeLists.txt"), 9))
            << QString();
    ...
    QTest::newRow("xmake error2")
            ...
            << (Tasks()
                << BuildSystemTask(Task::Error,
                                   ...
                                   "xmake may be not be built correctly."))
            << QString();

    QTest::newRow("xmake error at")
            << QString::fromLatin1("XMake Error at XMakeLists.txt:4:\n"
                                   "  Parse error.  Expected \"(\", got newline with text \"\n"
                                   "\n"
                                   "  \".\n")
            << OutputParserTester::STDERR
            ...
            << (Tasks()
                << BuildSystemTask(Task::Error,
                                   ...
                                   "\n"
                                   "\".",
                                   FilePath::fromUserInput("XMakeLists.txt"), 4))
            << QString();

    QTest::newRow("xmake syntax warning")
            << QString::fromLatin1("Syntax Warning in xmake code at\n"
                                   "/test/path/XMakeLists.txt:9:15\n"
    ...
    QTest::newRow("xmake warning")
            << QString::fromLatin1("XMake Warning at XMakeLists.txt:13 (message):\n"
                                   "  this is a warning\n\n")
            << OutputParserTester::STDERR
            << QString() << QString()
            << (Tasks()
                << BuildSystemTask(Task::Warning,
                                   "this is a warning",
                                   FilePath::fromUserInput("XMakeLists.txt"), 13))
            << QString();

    QTest::newRow("xmake author warning")
            << QString::fromLatin1("XMake Warning (dev) at XMakeLists.txt:15 (message):\n"
            ...
            << (Tasks()
                << BuildSystemTask(Task::Warning,
                                   "this is an author warning",
                                   FilePath::fromUserInput("XMakeLists.txt"), 15))
            << QString();

    QTest::newRow("eat normal XMake output")
    ...
    QTest::newRow("xmake call-stack")
        << QString::fromLatin1(
               "XMake Error at /Qt/6.5.3/mingw_64/lib/xmake/Qt6Core/Qt6CoreMacros.xmake:588 "
               "(add_executable):\n"
               "\n"
               "  Cannot find source file:\n"
               "\n"
               ...
               "  /Qt/6.5.3/mingw_64/lib/xmake/Qt6Core/Qt6CoreMacros.xmake:549 "
               "(_qt_internal_create_executable)\n"
               "  /Qt/6.5.3/mingw_64/lib/xmake/Qt6Core/Qt6CoreMacros.xmake:741 "
               "(qt6_add_executable)\n"
               "  /Projects/Test-Project/XMakeLists.txt:13 (qt_add_executable)\n")
        ...
        << (Tasks() << BuildSystemTask(
                ...
                "  /Qt/6.5.3/mingw_64/lib/xmake/Qt6Core/Qt6CoreMacros.xmake:549 "
                "(_qt_internal_create_executable)\n"
                "  /Qt/6.5.3/mingw_64/lib/xmake/Qt6Core/Qt6CoreMacros.xmake:588 "
                "(add_executable)",
                FilePath::fromUserInput("/Projects/Test-Project/XMakeLists.txt"),
                13))
        << QString();
}

void XMakeParserTest::testXMakeParser()
{
    OutputParserTester testbench;
    testbench.addLineParser(new XMakeParser);
...
QObject *createXMakeParserTest()
{
    return new XMakeParserTest;
}
