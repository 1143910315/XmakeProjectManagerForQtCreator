...
public:
    ...
    explicit XMakeTool(Detection d, const Utils::Id &id);
    explicit XMakeTool(const Utils::Store &map, bool fromSdk);
    ~XMakeTool();
    ...
    static Utils::FilePath xmakeExecutable(const Utils::FilePath &path);
    ...
    static void openXMakeHelpUrl(const XMakeTool *tool, const QString &linkUrl);

private:
    void readInformation() const;

    void runXMake(Utils::Process &proc, const QStringList &args, int timeoutS = 1) const;
    ...
    // Note: New items here need also be handled in XMakeToolItemModel::apply()
