...
XMakeInputsNode::XMakeInputsNode(const FilePath &xmakeLists) :
    ProjectExplorer::ProjectNode(xmakeLists)
{
    setPriority(Node::DefaultPriority - 10); // Bottom most!
    setDisplayName(Tr::tr("XMake Modules"));
    setIcon(DirectoryIcon(ProjectExplorer::Constants::FILEOVERLAY_MODULES));
    setListInProject(false);
}

XMakePresetsNode::XMakePresetsNode(const FilePath &projectPath) :
    ProjectExplorer::ProjectNode(projectPath)
{
    setPriority(Node::DefaultPriority - 9);
    setDisplayName(Tr::tr("XMake Presets"));
    setIcon(DirectoryIcon(ProjectExplorer::Constants::FILEOVERLAY_PRODUCT));
    setListInProject(false);
}

XMakeListsNode::XMakeListsNode(const FilePath &xmakeListPath) :
    ProjectExplorer::ProjectNode(xmakeListPath)
{
    setIcon(DirectoryIcon(Constants::Icons::FILE_OVERLAY));
    setListInProject(false);
}

bool XMakeListsNode::showInSimpleTree() const
{
    return false;
}
...
{
    return filePath().pathAppended(Constants::PROJECT_FILE_NAME);
}
...
QString XMakeProjectNode::tooltip() const
{
    return QString();
}

XMakeTargetNode::XMakeTargetNode(const FilePath &directory, const QString &target) :
    ProjectExplorer::ProjectNode(directory)
{
    m_target = target;
...
QString XMakeTargetNode::tooltip() const
{
    return m_tooltip;
}

QString XMakeTargetNode::buildKey() const
{
    return m_target;
}

FilePath XMakeTargetNode::buildDirectory() const
{
    return m_buildDirectory;
}

void XMakeTargetNode::setBuildDirectory(const FilePath &directory)
{
    m_buildDirectory = directory;
}

QVariant XMakeTargetNode::data(Id role) const
{
    auto value = [this](const QByteArray &key) -> QVariant {
        for (const XMakeConfigItem &configItem : m_config) {
    ...
    if (role == Ios::Constants::IosBuildDir) {
        ...
        // When generating Xcode project, XMake may put here a "${EFFECTIVE_PLATFORM_NAME}" macro,
...
void XMakeTargetNode::setConfig(const XMakeConfig &config)
{
    m_config = config;
}
...
void XMakeTargetNode::build()
{
    ...
    if (t)
        static_cast<XMakeBuildSystem *>(t->buildSystem())->buildXMakeTarget(displayName());
}

void XMakeTargetNode::setTargetInformation(const QList<FilePath> &artifacts, const QString &type)
{
