...
const char IOS_AUTOMATIC_PROVISIONG_UPDATES_ARGUMENTS_KEY[] =
        "XMakeProjectManager.MakeStep.iOSAutomaticProvisioningUpdates";
const char CLEAR_SYSTEM_ENVIRONMENT_KEY[] = "XMakeProjectManager.MakeStep.ClearSystemEnvironment";
const char USER_ENVIRONMENT_CHANGES_KEY[] = "XMakeProjectManager.MakeStep.UserEnvironmentChanges";
const char BUILD_PRESET_KEY[] = "XMakeProjectManager.MakeStep.BuildPreset";
...
XMakeTargetItem::XMakeTargetItem(const QString &target, XMakeBuildStep *step, bool special)
    : m_target(target), m_step(step), m_special(special)
{
}

QVariant XMakeTargetItem::data(int column, int role) const
{
    if (column == 0) {
...
bool XMakeTargetItem::setData(int column, const QVariant &data, int role)
{
...
Qt::ItemFlags XMakeTargetItem::flags(int) const
{
    ...
    XMakeAbstractProcessStep(bsl, id)
{
    ...
    if (XMakeBuildConfiguration::isIos(kit) && XMakeGeneratorKitAspect::generator(kit) == "Xcode") {
    ...
    connect(target(), &Target::activeRunConfigurationChanged,
            this, &XMakeBuildStep::updateBuildTargetsModel);
}

void XMakeBuildStep::toMap(Utils::Store &map) const
{
    XMakeAbstractProcessStep::toMap(map);
...
void XMakeBuildStep::fromMap(const Utils::Store &map)
{
...
bool XMakeBuildStep::init()
{
    if (!XMakeAbstractProcessStep::init())
        return false;
    ...
    setIgnoreReturnValue(m_buildTargets == QStringList(XMakeBuildStep::cleanTarget()));

    return true;
}

void XMakeBuildStep::setupOutputFormatter(Utils::OutputFormatter *formatter)
{
    ...
    formatter->addLineParsers({xmakeParser, new GnuMakeParser});
...
GroupItem XMakeBuildStep::runRecipe()
{
    const auto onParserSetup = [this](QPointer<Target> &parseTarget) {
        // Make sure XMake state was written to disk before trying to build:
        auto bs = qobject_cast<XMakeBuildSystem *>(buildSystem());
        QTC_ASSERT(bs, return SetupResult::StopWithError);
        QString message;
        if (bs->persistXMakeState())
            message = Tr::tr("Persisting XMake state...");
        else if (bs->isWaitingForParse())
            message = Tr::tr("Running XMake in preparation to build...");
        else
...
QString XMakeBuildStep::defaultBuildTarget() const
{
...
bool XMakeBuildStep::isCleanStep() const
{
    const BuildStepList *const bsl = stepList();
...
QStringList XMakeBuildStep::buildTargets() const
{
    return m_buildTargets;
}

bool XMakeBuildStep::buildsBuildTarget(const QString &target) const
{
    return m_buildTargets.contains(target);
}

void XMakeBuildStep::setBuildsBuildTarget(const QString &target, bool on)
{
...
void XMakeBuildStep::setBuildTargets(const QStringList &buildTargets)
{
...
CommandLine XMakeBuildStep::xmakeCommand() const
{
    CommandLine cmd{xmakeExecutable()};

    FilePath buildDirectory = ".";
    if (buildConfiguration())
        buildDirectory = buildConfiguration()->buildDirectory();

    cmd.addArgs({"--build", XMakeToolManager::mappedFilePath(buildDirectory).path()});
...
QString XMakeBuildStep::cleanTarget() const
{
    return QString("clean");
}

QString XMakeBuildStep::allTarget() const
{
    return m_allTarget;
}

QString XMakeBuildStep::installTarget() const
{
    return m_installTarget;
}

QStringList XMakeBuildStep::specialTargets(bool allCapsTargets)
{
    if (!allCapsTargets)
...
QString XMakeBuildStep::activeRunConfigTarget() const
{
    RunConfiguration *rc = target()->activeRunConfiguration();
    return rc ? rc->buildKey() : QString();
}

void XMakeBuildStep::setBuildPreset(const QString &preset)
{
    m_buildPreset = preset;
}

QWidget *XMakeBuildStep::createConfigWidget()
{
    auto updateDetails = [this] {
        ...
        if (!m_buildPreset.isEmpty()) {
            const XMakeProject *cp = static_cast<const XMakeProject *>(project());
    ...
    auto createAndAddEnvironmentWidgets = [this](Layouting::Form &builder) {
        ...
        connect(this, &XMakeBuildStep::environmentChanged, this, [this, envWidget] {
            envWidget->setBaseEnvironment(baseEnvironment());
            envWidget->setBaseEnvironmentText(baseEnvironmentText());
        });
    ...
    connect(&xmakeArguments, &BaseAspect::changed, this, updateDetails);
    ...
    connect(this, &XMakeBuildStep::buildTargetsChanged, widget, updateDetails);

    return widget;
}

void XMakeBuildStep::recreateBuildTargetsModel()
{
    auto addItem = [this](const QString &target, bool special = false) {
        auto item = new XMakeTargetItem(target, this, special);
...
void XMakeBuildStep::updateBuildTargetsModel()
{
    emit m_buildTargetModel.layoutChanged();
    emit buildTargetsChanged();
}

void XMakeBuildStep::setConfiguration(const QString &configuration)
{
    m_configuration = configuration;
}

void XMakeBuildStep::setToolArguments(const QStringList &nativeToolArguments)
{
    toolArguments.setValue(nativeToolArguments.join(" "));
}

void XMakeBuildStep::setXMakeArguments(const QStringList &xmakeArgs)
{
    xmakeArguments.setValue(xmakeArgs.join(" "));
}

Environment XMakeBuildStep::environment() const
{
    return m_environment;
}

void XMakeBuildStep::setUserEnvironmentChanges(const Utils::EnvironmentItems &diff)
{
...
EnvironmentItems XMakeBuildStep::userEnvironmentChanges() const
{
    return m_userEnvironmentChanges;
}

bool XMakeBuildStep::useClearEnvironment() const
{
    return m_clearSystemEnvironment;
}

void XMakeBuildStep::setUseClearEnvironment(bool b)
{
...
void XMakeBuildStep::updateAndEmitEnvironmentChanged()
{
...
Environment XMakeBuildStep::baseEnvironment() const
{
    Environment result;
...
QString XMakeBuildStep::baseEnvironmentText() const
{
    if (useClearEnvironment())
...
QString XMakeBuildStep::currentInstallPrefix() const
{
    auto bs = qobject_cast<XMakeBuildSystem *>(buildSystem());
...
void XMakeBuildStep::updateDeploymentData()
{
    if (!useStaging())
        return;

    QString install = currentInstallPrefix();
    FilePath rootDir = xmakeExecutable().withNewPath(stagingDir().path());
    Q_UNUSED(install);
...
class XMakeBuildStepFactory final : public BuildStepFactory
{
public:
    XMakeBuildStepFactory()
    {
...
{
    static XMakeBuildStepFactory theXMakeBuildStepFactory;
}

} // XMakeProjectManager::Internal

#include <xmakebuildstep.moc>
