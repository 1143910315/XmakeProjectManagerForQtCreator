...
void XMakeProcess::run(const BuildDirParameters &parameters, const QStringList &arguments)
{
    ...
    commandLine.addArgs({"-S",
                         XMakeToolManager::mappedFilePath(sourceDirectory).path(),
                         "-B",
                         XMakeToolManager::mappedFilePath(buildDirectory).path()});
...
{
    return Utils::transform(list, [](const QString &str) { return addXMakePrefix(str); });
}
